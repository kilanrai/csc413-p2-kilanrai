 ...
 * Adding and Getting bytecodes will go through the Program class. This class also loads
 * bytecode objects into a Program instance.
 */
        ...
        try {
            while ((line = byteSource.readLine()) != null) {
                ...
                if (!line.isEmpty()) { // not an empty line
                    ...
                    // We retrieve the class name from the CodeTable and store on the className as string
                    String className = CodeTable.getClassName(args[0]);
                    Class c = Class.forName("interpreter.bytecode." + className);

                    //We create an instance for the given class blueprint.
